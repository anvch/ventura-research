...
points1 = [(1, 1), (2, 2), (3, 1), (4, 2), (2, 4), (2, 3), (1, 2)]
messyPoly = [
            LineString([(1, 1), (3, 1)]),
            LineString([(3, 1), (4, 2)]),
            LineString([(4, 2), (1, 2)]),
...
points2 = [(3, 1), (1, 4), (1, 1), (2, 2), (5, 1), (5, 4), (3, 4), (2, 3)]

def filter(oldTris, newTris):
    valid_polys = []
    for tri in newTris:
        edges = [    
        ...
        try:
            # try to create a polygon from the triangle edges
            tri_polygon = list(polygonize(edges))[0]
            # check if the original polygon contains or overlaps with the new triangle
            if oldTris.overlaps(tri_polygon) or oldTris.contains(tri_polygon):
                valid_polys.append(edges)

        except IndexError:
            # If polygonize returns an empty list (invalid polygon)
            print(f"Warning: Could not create polygon from face {face}")
            continue

def delaunay(points, oldPoly):
    ...
    for tri in tris.simplices:
        face = []
        for tri_pt in tri:
            # tri_pt is the index into points
            face.append(points[int(tri_pt)])

        # create list of linestrings for each edge of the triangle
        edges = [    
            LineString([face[0], face[1]]),
            LineString([face[1], face[2]]),
            LineString([face[2], face[0]])
            ]
        ...
        try:
            ...
            # check if the original polygon contains or overlaps with the new triangle
            if oldPoly.overlaps(tri_polygon) or oldPoly.contains(tri_polygon):
                valid_polys.append(edges)
...
def plotMultipolygon(delaunayPoly, oldPoly, xlim, ylim):
    ...
    count = 0
    # delaunay
    for poly in delaunayPoly:
        for edges in poly:
            x, y = edges.xy
            ax.plot(x, y, color = 'blue', linewidth = 2,  label='Delaunay triangles' if count  == 0 else "")
            count += 1

    # original (should be one polygon)
    count = 0
    for edge in oldPoly:
        x, y = edge.xy
        ax.plot(x, y, color='red', linewidth=2, linestyle='--', label='Old polygon boundary' if count  == 0 else "")
        count += 1

    # Formatting the plot
    ax.set_title(f"Multipoly Visualization")
    ax.set_aspect('equal')
...
def main():
    # basicTestSuite()
    print("DELAUNEY")
    plotMultipolygon(delaunay(points2, list(polygonize(b1))[0]), b1, (-1, 6), (-1, 6))

    # load old triangles from uv layer 5
    # print(f"loading old triangles from layer: 5")
    # old_triangles = []
    # with open("halde_outer_polygon_nomax_layer_5.txt", "r") as f:
    #     for line in f:
    #         triangle = ast.literal_eval(line.strip())
    #         # e1 = LineString([triangle[0], triangle[1]])
    #         # e2 = LineString([triangle[1], triangle[2]])
    #         # e3 = LineString([triangle[2], triangle[0]])
    #         # convert triangle to shapely polygon
    #         triangle = Polygon(triangle)
    #         # old_triangles.append(e1)
    #         # old_triangles.append(e2)
    #         # old_triangles.append(e3)
    #         old_triangles.append(triangle)

    # new_triangles = []
    # with open("halde_new_edges_layer_5_pix.txt") as f:
    #     for line in f:
    #         new_tri = ast.literal_eval(line.strip())
    #         new_triangles.append(new_tri)
    #         print(new_tri)
    # # print(list(polygonize(old_triangles))[0])
    # filter(old_triangles, new_triangles)
    # # print(list(old_triangles.boundary.coords))
